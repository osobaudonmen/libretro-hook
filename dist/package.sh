...
if [[ -f "$ROOT_DIR/main/generated/unix/hook_libretro.so" ]]; then
  mkdir -p "$CORE_DIR/unix"; cp "$ROOT_DIR/main/generated/unix/hook_libretro.so" "$CORE_DIR/unix/";
fi
# Windows
if [[ -f "$ROOT_DIR/main/generated/wincross64/hook_libretro.dll" ]]; then
  mkdir -p "$CORE_DIR/windows-x64"; cp "$ROOT_DIR/main/generated/wincross64/hook_libretro.dll" "$CORE_DIR/windows-x64/";
fi
# Android
for ABI in armeabi-v7a arm64-v8a; do
  if [[ -f "$ROOT_DIR/main/generated/$ABI/hook_libretro.so" ]]; then
    mkdir -p "$CORE_DIR/android-$ABI"; cp "$ROOT_DIR/main/generated/$ABI/hook_libretro.so" "$CORE_DIR/android-$ABI/";
  fi
done
