...
- 現状Androidは子プロセスやインテントを用いたRetroArchの起動ができないため正常に動作しません。配布用のバイナリやアセット、ダウンロードからは除外してください。
...
- `retroarch`コマンドを子プロセスで起動後は`exit(0)`で終了します。
...
- Systemディレクトリは、ゲーム別の設定ファイルを保存する場所を決定するために使用します。
  - 多くの場合、`<Systemディレクトリ>/../config/<CORE_NAME>/<ゲーム名>.cfg`がゲーム別の設定ファイルパスになります。
  - 既にゲーム別の設定ファイルが存在する場合は、上書きしません。
...
- 使用するコアを判定する方法はROMファイルパスに含まれる文字列で判定します。次に例を示します。
  ...
  - パスから判定できなかった場合は、`<core:コアファイル>`を出力しない。
